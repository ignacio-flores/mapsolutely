...
map_osm_features <- function(
  longitude, latitude, buffer_radius, features_colors, other_colors, 
  crop_type = "vertical_2_3", highlight_osm_ids = NULL, title = NULL, 
  subtitle = NULL, save_path = NULL, dpi = 300, outer_margin = 0.05) {
  ...
  for (feature in names(features_colors)) {
    ...
    if (startsWith(feature, "highway.")) {
      highway_type <- sub("highway.", "", feature)  # Extract subkey (e.g., "residential")
      osm_data <- opq(bbox = bbox) %>%
        add_osm_feature(key = "highway", value = highway_type) %>%
        osmdata_sf()
    } else {
      osm_data <- opq(bbox = bbox) %>%
  ...
  for (feature in names(data_list)) {
    if (!is.null(data_list[[feature]])) {
      if (data_list[[feature]]$type == "polygon") {
        ...
        tm <- tm + tm_shape(data_list[[feature]]$geometry) +
          tm_lines(col = features_colors[[feature]], lwd = 4)
      }
    }
  }
...
# Function to download specific OSM data by feature type
download_osm_data <- function(latlong, buffer_radius, feature_type = NULL) {
  library(osmdata)
  coords <- as.numeric(strsplit(latlong, ",")[[1]])
  bbox <- opq(bbox = c(coords[2] - 0.01, coords[1] - 0.01, coords[2] + 0.01, coords[1] + 0.01))
  if (!is.null(feature_type)) {
    bbox <- add_osm_feature(bbox, key = feature_type)
  }
  osm_data <- osmdata_sf(bbox)
